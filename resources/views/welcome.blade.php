<x-layout>
    ...
    <div class="hero-section text-center text-white"
        style="background-image: url('path/to/your/image.jpg'); height: 80vh; background-size: cover; background-position: center;">
                    ...
                    <div class="card shadow-sm border-0" style="border-radius: 15px overflow: hidden;" data-aos="fade-up"
                        ...
                        @if ($article->category)
                            ...
                            <div class="card-footer" style="background-color: #f9f9f9;">
                                ...
                                <a href="{{ route('article.byCategory', $article->category) }}" class="text-capitalize text-muted badge bg-secondary">{{ $article->category->name }} >
                        @else
                        ...
                        @endif
                            ...
                            @if ($article->category)
                                ...
                                <!-- Categoria -->
                                <span class="badge badge-pill badge-primary mb-2">{{ route('article.byCategory', $article->category) }}" class="text-capitalize text-muted">{{ $article->category->name }}</span>
                            @else
                                <p class="small text-muted">Nessuna categoria</p>
                            @endif

                            <p class="card-text"><small class="text-muted">Scritto da <a
                                        href="{{ route('article.byUser', $article->user) }}"
    ...
    <div class="newsletter-section text-center my-5 py-5" style="background-color: #f5f5f5;">
        ...
        <form action="/subscribe" method="POST">
            @csrf
            <input type="email" name="email" placeholder="Mail" required class="form-control my-3"
